<template>
  <div class="panel">
    <div
      class="grid grid--v-spread grid--s-24"
      :class="$mq == 'ptab' || $mq == 'mob' ? 'grid--vertical' : 'grid--center'"
    >
      <div
        class="grid__item"
        :class="{ 'one-half': $mq == 'ltab' || $mq == 'desk' || $mq == 'max' }"
      >
        ...
        <div
          ...
          :class="{
            'panel__content--vertical':
              $mq == 'mob' || $mq == 'ptab' || $mq == 'max',
            'panel__content--list': isList
          }"
        >
          <slot />
          <div v-if="highlight" class="panel__highlight">
            {{ this.highlight }}
          </div>
        </div>
      </div>
